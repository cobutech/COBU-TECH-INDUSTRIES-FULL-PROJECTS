const BASE_DATA_PAGE = 'administration-networks-data-page.html';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    quickActionTarget.innerHTML = `
      <div class="quick-action-wrapper glowing-container">
        ...
        <div id="quick-action-dropdown" class="quick-action-dropdown">
          ...
          <a href="${BASE_DATA_PAGE}?page=act-1" class="qa-dropdown-item">Activation</a>
          <a href="${BASE_DATA_PAGE}?page=cust-2" class="qa-dropdown-item">Customers</a>
          <a href="${BASE_DATA_PAGE}?page=act-2" class="qa-dropdown-item">Prepaid Users</a>
          <a href="${BASE_DATA_PAGE}?page=net-3" class="qa-dropdown-item">Routers</a>
          <a href="${BASE_DATA_PAGE}?page=act-5" class="qa-dropdown-item">Online Users</a>
          <a href="${BASE_DATA_PAGE}?page=set-8" class="qa-dropdown-item">Active Sessions</a>
        </div>
      </div>
    `;
  ...
  // --- 2. SIDEBAR MENU DATA (ALL INITIALLY UNMARKED) ---
  const sidebarItemsData = [
    ...
    { id: "act", title: "Activation", icon: `<polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/>`, children: [
      ...
      { id: "act-8", title: "FUP Users", starred: false },
    ...
    { id: "cust", title: "Customers", icon: `<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/>`, children: [
      { id: "cust-1", title: "Add New User", starred: false },
      { id: "cust-2", title: "Users", starred: false },
    ...
    { id: "pkg", title: "Packages/Plans", icon: `<path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/>`, children: [
      ...
      { id: "pkg-4", title: "Bandwidth Plans", starred: false },
      ...
      { id: "pkg-10", title: "TV Plans", starred: false }
    ]},
    { id: "tx", title: "Transactions", icon: `<line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/>`, children: [
      ...
      { id: "tx-3", title: "Activation History", starred: false },
    ...
    { id: "net", title: "Network", icon: `<path d="M6 3v12"/><path d="M18 9a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/><path d="M6 21a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>`, children: [
      ...
      { id: "net-3", title: "Routers", starred: false },
    ...
    { id: "settings", title: "Settings", icon: `<circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82"/>`, children: [
      ...
      { id: "set-7", title: "Payment Gateway", starred: false },
    ...
    { id: "pppoe-set", title: "PPPoE Settings", icon: `<path d="M18 20V6a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v14"/>`, children: [
      { id: "pps-1", title: "Setup", starred: false },
    ...
    for (const cat of sidebarItemsData) {
      ...
      for (const child of cat.children) {
        if (child.isNested && child.children) {
          ...
          if (nestedMatch) {
            nestedMatch.parentId = cat.id;
            nestedMatch.nestedParentId = child.id;
            return nestedMatch;
          }
  ...
  function renderSidebar() {
    ...
    sidebarItemsData.forEach((item) => {
      ...
      const href = item.target
        ? item.target
        : (hasChildren || item.isFavoritesMenu
          ? '#'
          : `${BASE_DATA_PAGE}?page=${item.id}`);
        ...
        item.children.forEach(child => {
          if (child.isNested) {
              ...
              sidebarHTML += `
                <li>
                  <div class="submenu-item-row">
                    <a href="${BASE_DATA_PAGE}?page=${nChild.id}" class="${nChild.id === activePageId ? 'active' : ''}">${nChild.title}</a>
            ...
            sidebarHTML += `
              <li>
                <div class="submenu-item-row">
                  <a href="${BASE_DATA_PAGE}?page=${child.id}" class="${child.id === activePageId ? 'active' : ''}">
                    ${child.title}
  ...
  function autoExpandActiveParent() {
    ...
    if (activeItem.nestedParentId) {
      ...
      if (nestedLi) {
        nestedLi.classList.add('expanded');
      }
    }
  ...
  function renderFavoritesList() {
    ...
    if (starredItems.length === 0) {
      favSubmenu.innerHTML = `<li class="empty-favs" style="padding: 10px 15px; font-size: 0.8rem; color: #888;">No favorites added yet</li>`;
      return;
    }
    ...
    starredItems.forEach(item => {
      favHTML += `
        <li>
          <div class="submenu-item-row">
            <a href="${BASE_DATA_PAGE}?page=${item.id}" class="${item.id === activePageId ? 'active' : ''}">${item.title}</a>
    ...
    // Attach deletion handlers to Favorites submenu items
    favSubmenu.querySelectorAll('.fav-remove-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (targetItem) {
          ...
          // Un-highlight corresponding star icons across DOM
          document.querySelectorAll(`.star-btn[data-id="${id}"]`).forEach(star => {
  ...
  function attachNavigationListeners() {
    const topLinks = document.querySelectorAll(
      '.sidebar-link[data-has-dropdown="true"], .sidebar-item[data-item-id="favs"] > .sidebar-link'
    );
    ...
    // Star Button Toggle (with 8-item limit enforcement)
    ...
    starBtns.forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        if (item) {
          if (!item.starred) {
            if (getStarredCount() >= 8) {
              showToast('Maximum 8 favorites allowed! Remove an item to add a new one.');
              return;
            }
          ...
          // Sync star button UI state across the entire document
          document.querySelectorAll(`.star-btn[data-id="${id}"]`).forEach(star => {
